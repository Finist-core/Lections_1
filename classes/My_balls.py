#TODO ни хера не понятно, повторить и понять!!!
...
class Ball:
    ...
    def __init__(self):
        ...
        self.y = randint(self.R, HEIGH - self.R)
        self.dx, self.dy = (+2, +2)
    ...
    def move(self):
        ...
        if self.x + self.R > WIDTH or self.x - self.R <= 0:
            self.dx = - self.dx
        if self.y + self.R > HEIGH or self.y - self.R <= 0:
            self.dy = - self.dy
...
def main():
    ...
    root = tk.Tk()
    root.geometry(str(WIDTH) + 'x' + str(HEIGH))
